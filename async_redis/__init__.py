...
from tornado import ioloop, gen, stack_context, version_info
...
class ModIOStream(IOStream):
    def read_bytes(self, num_bytes, callback=None, streaming_callback=None, partial=False):
        self._read_partial = True
        if version_info[0] > 3:
            super(ModIOStream, self).read_bytes(num_bytes, callback, streaming_callback, partial)
        else:
